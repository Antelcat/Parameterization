		...
		foreach (var (syntaxContext, attributeSyntax) in targets)
		{
			...
			foreach (var parameterType in parameterTypes)
			{
				convertersGenerator.AddConverter(
					parameterType, $"{Global.TypeDescriptor}.GetConverter(typeof({parameterType}))");
			}
			foreach (var converterType in converterTypes)
			{
				convertersGenerator.AddConverter(
					converterType, $"new {converterType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}()");
			}
